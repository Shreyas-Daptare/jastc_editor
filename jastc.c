...
#define JASTC_VERSION "0.0.1"
#define JASTC_TAB_STOP 8
#define JASTC_QUIT_TIMES 3
...
int editorReadKey() //read the keypress input
{
	int nread;
  	char c;
...
int getWindowSize(int *rows, int *cols) //To get window size using ioctl and if ioctl fails use the fallback method
{
	struct winsize ws;
...
int editorRowCxToRx(erow *row, int cx)
{
	...
	{
		if (row->chars[j] == '\t')
			rx += (JASTC_TAB_STOP - 1) - (rx % JASTC_TAB_STOP);
		rx++;
	}
	return rx;
}
void editorUpdateRow(erow *row)
{
	...
	free(row->render);
  	row->render = malloc(row->size + tabs*(JASTC_TAB_STOP - 1) + 1);
...
void editorDrawRows(struct abuf *ab)  // Draw tildes(~) in the windows
{
	int y;
	for (y = 0; y < E.screenrows; y++)
	{
		...
		{
		if (E.numrows==0 && y == E.screenrows / 3) //Print the Editor name and version in the center of the screen with padding
		{
			char welcome[80];
			int welcomelen = snprintf(welcome, sizeof(welcome), "JASTC Editor -- version %s", JASTC_VERSION);
		...
		abAppend(ab, "\x1b[K", 3); //erase the part of the line to the right of the cursor rather than clearing the screen
		abAppend(ab, "\r\n", 2);
	}
}
...
void editorRefreshScreen()
{
	...
	abAppend(&ab, "\x1b[?25l", 6); //Hide the cursor before drawing the tildes
	//abAppend(&ab, "\x1b[2J", 4); //Clear the screen
	abAppend(&ab, "\x1b[H", 3); //Reposition the cursor back to the first line and column of the terminal
	...
	abAppend(&ab, "\x1b[?25h", 6); //Show the cursor after drawing the tildes
...
void editorMoveCursor(int key) 
{
	...
	switch (key) 
	{
		/* The if conditions are for keeping the cursor position in range */
		case ARROW_LEFT:
			if(E.cx != 0)
			{
				E.cx--;
...
{
	static int quit_times = JASTC_QUIT_TIMES;
